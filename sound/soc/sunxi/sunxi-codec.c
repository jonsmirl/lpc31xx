...
#define regmap_update_bitsx(map, reg, mask, val) {printk("JDS CDC reg %x mask %x val %x\n", reg,mask,val);regmap_update_bits(map, reg, mask, val);}
...
#endif
	regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x1 << PA_MUTE, 0x0 << PA_MUTE);
	mdelay(5);
	//disable dac drq
	regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 0x1 << DAC_DRQ, 0x0 << DAC_DRQ);
	//pa mute
	regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x1 << PA_MUTE, 0x0 << PA_MUTE);
	regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x1 << DACAEN_L, 0x0 << DACAEN_L);
	regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x1 << DACAEN_R, 0x0 << DACAEN_R);
	return 0;
}
...
static int codec_capture_start(struct sunxi_priv *priv)
{
	...
	if (gpio_pa_shutdown)
		gpio_write_one_pin_value(gpio_pa_shutdown, 1, "audio_pa_ctrl");
#endif
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0x1 << ADC_DRQ, 0x1 << ADC_DRQ);
	return 0;
}
...
{
	//disable adc drq
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0x1 << ADC_DRQ, 0x0 << ADC_DRQ);
	//enable mic1 pa
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_ACTL, 0x1 << MIC1_EN, 0x0 << MIC1_EN);
	//enable VMIC
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_ACTL, 0x1 << VMIC_EN, 0x0 << VMIC_EN);
	if (priv->id == SUN7I) {
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_TUNE, 0x3 << 8, 0x0 << 8);
	}
	//enable adc digital
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0x1 << ADC_DIG_EN, 0x0 << ADC_DIG_EN);
	//set RX FIFO mode
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0x1 << RX_FIFO_MODE, 0x0 << RX_FIFO_MODE);
	//flush RX FIFO
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0x1 << ADC_FIFO_FLUSH, 0x0 << ADC_FIFO_FLUSH);
	//enable adc1 analog
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_ACTL, 0x3 << ADC_EN, 0x0 << ADC_EN);
	return 0;
}
...
static int sunxi_codec_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK){
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_DPC, 0x1 << DAC_EN, 0x1 << DAC_EN);
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 0x1 << DAC_FIFO_FLUSH, 0x1 << DAC_FIFO_FLUSH);
		//set TX FIFO send drq level
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 0x3f << TX_TRI_LEVEL, 0xf << TX_TRI_LEVEL);
		if (substream->runtime->rate > 32000) {
			regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 0x1 << 28, 0x0 << 28);
		} else {
			regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 0x1 << 28, 0x1 << 28);
		}
		//set TX FIFO MODE
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 0x1 << TX_FIFO_MODE, 0x1 << TX_FIFO_MODE);
		//send last sample when dac fifo under run
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 0x1 << LAST_SE, 0x0 << LAST_SE);
		//enable dac analog
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x1 << DACAEN_L, 0x1 << DACAEN_L);
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x1 << DACAEN_R, 0x1 << DACAEN_R);
		//enable dac to pa
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x1 << DACPAS, 0x1 << DACPAS);
	} else {
		//enable mic1 pa
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_ACTL, 0x1 << MIC1_EN, 0x1 << MIC1_EN);
		//mic1 gain 32dB
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_ACTL, 0x3 << 25, 0x1 << 25);
		//enable VMIC
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_ACTL, 0x1 << VMIC_EN, 0x1 << VMIC_EN);

		if (priv->id == SUN7I) {
			/* boost up record effect */
			regmap_update_bitsx(priv->regmap, SUNXI_DAC_TUNE, 0x3 << 8, 0x1 << 8);
		}

		//enable adc digital
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0x1 << ADC_DIG_EN, 0x1 << ADC_DIG_EN);
		//set RX FIFO mode
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0x1 << RX_FIFO_MODE, 0x1 << RX_FIFO_MODE);
		//flush RX FIFO
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0x1 << ADC_FIFO_FLUSH, 0x1 << ADC_FIFO_FLUSH);
		//set RX FIFO rec drq level
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 0xf << RX_TRI_LEVEL, 0x7 << RX_TRI_LEVEL);
		//enable adc1 analog
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_ACTL, 0x3 << ADC_EN, 0x3 << ADC_EN);
	}
	return 0;
}
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 7 << 29, rate << 29);
		if (substream->runtime->channels == 1) {
			regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 1 << 6, 1 << 6);
		} else {
			regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 1 << 6, 0 << 6);
		}
	} else  {
		regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 7 << 29, rate << 29);
		if (substream->runtime->channels == 1) {
			regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 1 << 7, 1 << 7);
		} else {
			regmap_update_bitsx(priv->regmap, SUNXI_ADC_FIFOC, 1 << 7, 0 << 7);
		}
	}
	return 0;
}
...
{
	//enable dac digital
	regmap_update_bitsx(priv->regmap, SUNXI_DAC_DPC, 1 << DAC_EN, 1 << DAC_EN);

	regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 1 << 28, 1 << 28);
	//set digital volume to maximum
	if (priv->id == SUN4A)
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_DPC, 6 << DIGITAL_VOL, 0 << DIGITAL_VOL);
	//pa mute
	regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 1 << PA_MUTE, 0 << PA_MUTE);
	//enable PA
	regmap_update_bitsx(priv->regmap, SUNXI_ADC_ACTL, 1 << PA_ENABLE, 1 << PA_ENABLE);
	regmap_update_bitsx(priv->regmap, SUNXI_DAC_FIFOC, 3 << DRA_LEVEL, 3 << DRA_LEVEL);
	//set volume
	if ((priv->id == SUN4I) || (priv->id == SUN4A)) {
		int rc;
		int device_lr_change = 0;
		if (priv->id == SUN4A) {
			regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x3f << VOLUME, 1 << VOLUME);
		} else {
 			regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x3f << VOLUME, 0x3b << VOLUME);
		}
#ifdef JDS
		...
		if (device_lr_change)
			regmap_update_bitsx(priv->regmap, SUNXI_DAC_DEBUG, 1 << DAC_CHANNEL, 1 << DAC_CHANNEL);
#endif
	} else {
		regmap_update_bitsx(priv->regmap, SUNXI_DAC_ACTL, 0x3f << VOLUME, 0x3b << VOLUME);
	}
